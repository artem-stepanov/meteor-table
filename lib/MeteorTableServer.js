    ...
    methods['meteorTableDownloadExcelFile' + name] = function (fieldsExcel, columnsOrderExcel, shownColumnsExcel, limitExcel, skipExcel, sortExcel, sortOrderExcel, filtersExcel, dataFiltersExcel, selectorFiltersExcel, dateFormatMomentExcel) {
        ...
        mkdirp('tmp', Meteor.bindEnvironment(function (err) {
            if (err) {
                ...
                MeteorTable.excelFiles.importFile(filePath, {
                    _id: uuid,
                    filename: filename,
                    ...
                    if (err) {
                        ...
                        MeteorTable.excelFiles.update({_id: file._id}, {
                            $set: {
                                'metadata.owner': Meteor.userId()
                            }
                        });
                        futureResponse.return(process.env.ROOT_URL + '/gridfs/excelFiles/' + file._id + '/' + encodeURIComponent(filename));
                        Meteor.setTimeout(function () {
                            console.log("Remove file " + file._id);
                            MeteorTable.excelFiles.remove({_id: file._id});
                        }, 1800000);
                    }
                });
            }
...
Meteor.publish('excelFilesData',
    function (clientUserId) {
        if (clientUserId === this.userId) {
            return MeteorTable.excelFiles.find({'metadata.owner': this.userId});
        } else {
            return null;
        }
    }
