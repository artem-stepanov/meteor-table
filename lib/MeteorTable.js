...
MeteorTable.init = function (settings) {
    ...
    that.tableReady = new ReactiveVar(false);
};
...
MeteorTable.field = function (name, type, params) {
    ...
    this.requiredFilter = getParam("requiredFilter") || false;
    ...
    this.selectMultiple = getParam("selectMultiple");
            ...
            case MeteorTable.FIELD_TYPE_SELECT:
                ...
                _.each(options, function (option) {
                    if (_.has(option, 'optgroup')) {
                        if (_.has(optgroup, option.optgroup)) {
                            optgroup[option.optgroup].push(option);
                        } else {
                            optgroup[option.optgroup] = [option];
                        }
                    } else {
                        withoutoptgroup.push(option);
                    }
                });
                var sortOptgroup = getParam("sortOptgroup");
                if (sortOptgroup) {
                    var optgroupBySort = {};
                    _.each(sortOptgroup, function (optgroupName) {
                        if (!_.isEmpty(optgroup[optgroupName])) {
                            optgroupBySort[optgroupName] = optgroup[optgroupName];
                        }
                    });
                    optgroup = optgroupBySort;
                }
                optgroup.withoutoptgroup = withoutoptgroup;
                _.each(optgroup, function (options, optgroupName) {
                    if (optgroupName !== 'withoutoptgroup') {
                        select += ' <optgroup label="' + optgroupName + '">';
                    }
                    _.each(options, function (option) {
                        var selected;
                        if (_.isArray(filterValue)) {
                            selected = _.contains(filterValue, option.value) ? ' selected' : '';
                        } else {
                            selected = option.value == filterValue && filterValue !== "" ? ' selected' : '';
                        }
                        var title = _.isFunction(option.title) ? option.title.call(this) : _.isUndefined(option.title) ? "" : option.title;
