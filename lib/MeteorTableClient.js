...
var hideFilters = function () {
    Template.instance().$(".meteor-table .filters-show").css('visibility', 'visible');
    Template.instance().$(".meteor-table .filters-hide").hide();
};

var showFilters = function () {
    Template.instance().$(".meteor-table .filters-show").css('visibility', 'hidden');
...
var updatePublication = function (context) {
    ...
    var onReady = function () {
        if (publicationId === context.nextPublicationId.get()) {
            ...
            if (!_.isUndefined(context.data.settings, 'countFromData') && context.data.settings.countFromData) {
                const count = context.publishedRows.findOne({_id: 'count'})['count'];
                context.count.set(count);
            }
...
var emptyRequiredFilters = function (context, dataFilters) {
    const emptyFilters = [], settings = context.data.settings, filters = context.filters;
    if (!_.isUndefined(settings, 'hasRequiredFilters') && settings.hasRequiredFilters) {
        _.each(filters.get(), function (filter) {
            if (!_.isUndefined(filter.requiredFilter) && filter.requiredFilter && !dataFilters[filter.name]) {
                emptyFilters.push(filter.name);
            }
        });
    }
    return emptyFilters;
...
Template.meteorTable.helpers({
    ...
    isNoDblClick: function () {
        return _.has(Template.instance().data.settings, 'noDblClick') && Template.instance().data.settings.noDblClick;
    },
    ...
    rowsData: function () {
        ...
        const data = Template.instance().publishedRows.find({
    ...
    getPaginationArray: function () {
        var out = false;
        var pagesCount = Math.ceil(Template.instance().count.get() / Template.instance().rowPerPage.get());
    ...
    emptyFilters: function () {
        return Template.instance().emptyFilters.get().length;
    },
    isFillFilter: function () {
        return _.contains(Template.instance().emptyFilters.get(), this.name);
    },
    showRowPerPage: function () {
        return Template.instance().count.get() > 10;
    },
    tableReady: function () {
    ...
    getValue: function () {
        ...
        if ((_.isObject(view) && _.has(view, "string") && view.string === "") || view === "") {
    ...
    getLabel: function (labelName) {
        var labels = {
            ...
            fillFilters: "Fill required filters",
            columns: "Columns",
...
Template.meteorTable.onCreated(function () {
    ...
    if (_.has(db, 'noResponsiveTable')) {
        noResponsiveTable.set(db.noResponsiveTable);
    }
    ...
    var dataFilters = db.dataFilters || {};
    if (FlowRouter) {
        var filter = FlowRouter.getQueryParam("filter");
        if (filter) {
            try {
                dataFilters = JSON.parse(filter);
                var selectors = MeteorTable.getSelectors({}, filters.get(), dataFilters, context.dateFormatMoment);
                selectorFilters.set(selectors);
                updateDbData("dataFilters", dataFilters);
                updateDbData("selectorFilters", JSON.stringify(selectors));
            } catch (e) {
                console.log(e);
            }
        }
    }
    dataFilters = new ReactiveVar(dataFilters);
    ...
    context.loading = new ReactiveVar(false);

    context.firstLoad = new ReactiveVar(true);
    ...
    context.anyChecked = new ReactiveVar(false);

    context.tableReady = data.tableReady;
    ...
    context.totalChecked = new ReactiveVar(0);

    context.count = new ReactiveVar(0);

    context.emptyFilters = new ReactiveVar([]);

    context.autorun(function () {

        Session.set('meteor-table-current-page-' + settings.subscribe, page.get());

        // не загружать данные, если не заполнены обязательные фильтры
        const emptyFilters = emptyRequiredFilters(context, context.dataFilters.get());
        if (!emptyFilters.length) {
            updatePublication(context);
        } else {
            context.tableReady.set(true);
        }
        context.emptyFilters.set(emptyFilters);

        if (_.isUndefined(settings, 'countFromData') || !settings.countFromData) {
            context.count.set(Counts.get('meteor-table-count-' + Template.instance().data.settings.subscribe));
        }
...
Template.meteorTable.onRendered(function () {
    ...
    if (template.emptyFilters.get().length) {
        showFilters();
    }
...
Template.meteorTable.events({
    ...
    'click .meteor-table-filters .meteor-table-submit': function (e) {
        ...
        const emptyFilters = emptyRequiredFilters(Template.instance(), dataFilters);
        Template.instance().emptyFilters.set(emptyFilters);
        if (emptyFilters.length) {
            return;
        }
                ...
                case MeteorTable.FIELD_TYPE_SELECT:
                    if (Meteor.isCordova) {
                        ...
                        if (field.selectMultiple) {
                            $filterForm.find("select.meteor-table-select2[name='" + field.name + "']").val("All").trigger("change");
                        } else {
                            $filterForm.find("select.meteor-table-select2[name='" + field.name + "']").val("").trigger("change");
                        }
                    }
                    break;
    ...
    'click .setResponsiveTable': function (event, template) {
        updateDbData("noResponsiveTable", false);
        Template.instance().noResponsiveTable.set(false);
    },
    'click .setNoResponsiveTable': function (event, template) {
        updateDbData("noResponsiveTable", true);
        Template.instance().noResponsiveTable.set(true);
    }
});
