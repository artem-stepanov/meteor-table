...
var updatePublication = function (context) {
    ...
    var onReady = function () {
        if (publicationId === context.nextPublicationId.get()) {
            ...
            console.log('publicationId', newPublishedRows.find().count());
...
Template.meteorTable.helpers({
    ...
    rowsData: function () {
        ...
        //);
        //return Template.instance().data.settings.collections.find(selector, params);

        Template.instance().publishedRows.find({
            "reactive-table-id": Template.instance().publicationId.get()
        });
    },
    ...
    getValue: function () {
        ...
        return this.getView(this.name, fieldKey, value, object, Template.instance().dateFormatMoment) || new Spacebars.SafeString("&nbsp;");
    },
...
Template.meteorTable.onCreated(function () {
    ...
    var db = {};
    var meteorTableDb = Meteor.users.findOne({_id: Meteor.userId()}).meteorTable;
    if (!_.isUndefined(meteorTableDb)) {
        if (_.has(meteorTableDb, Template.instance().data.settings.subscribe)) {
            db = meteorTableDb[Template.instance().data.settings.subscribe];
        }
    }
    ...
    _.each(filters.get(), function (filter) {
        ...
        if (filter.type == MeteorTableParams.METEOR_TABLE_FIELD_TYPE_DATE) {
            var dataObjValue = {};
    ...
    var fields = this.data.settings.fields || {};
    var tmpField = {};
    _.each(fields, function (field) {
        tmpField[field.name] = field;
    });
    fields = new ReactiveVar(tmpField);
    this.fields = fields;
    ...
    this.publicationId = new ReactiveVar();
    this.nextPublicationId = new ReactiveVar();
    this.publishedRows = new Mongo.Collection(null);

    updatePublication(this);

    this.autorun(function () {

        if (i18n.getLanguage()) {
            var $select = $(".meteor-table select.meteor-table-select2");
...
Template.meteorTable.onRendered(function () {
    ...
    //template.$('.meteor-table select').select2({
    //    width: "100%"
    //});
    template.$('.meteor-table .input-daterange').datepicker({
