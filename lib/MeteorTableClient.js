...
Template.meteorTable.events({
    ...
    'click .meteor-table-table th': function (event) {
        ...
        if ($(event.target).closest("th:has(input.checkboxes)")) {
    ...
    'click .navbar-checkboxes-buttons button': function(e) {
        var name = $(e.currentTarget).attr("data-func-name");
        var ids = [];
        $(e.currentTarget).closest(".meteor-table").find(".meteor-table-table table td input.checkboxes:checked").each(function(){
        ...
        if (!_.isUndefined(method) && _.has(method, 'func') && _.isFunction(method.func)) {
            method['func'].call(Template.instance(), ids);
        }
    }
});
